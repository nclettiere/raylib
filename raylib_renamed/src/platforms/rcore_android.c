...
#include <jni.h>                        // Required for: JNIEnv and JavaVM [Used in rl_OpenURL() and rl_GetCurrentMonitor()]
...
static const rl_KeyboardKey mapKeycode[KEYCODE_MAP_SIZE] = {
...
static rl_GamepadButton AndroidTranslateGamepadButton(int button);                     // Map Android gamepad button to raylib gamepad button
...
int rl_GetCurrentMonitor(void)
{
    ...
    if (display == NULL)
    {
        TRACELOG(LOG_ERROR, "rl_GetCurrentMonitor() couldn't get the display object");
    }
    else
    {
...
static void AndroidCommandCallback(struct android_app *app, int32_t cmd)
{
    switch (cmd)
    {
        ...
        case APP_CMD_TERM_WINDOW:
        {
            ...
            // this means that the user has already called 'rl_CloseWindow()'
...
static rl_GamepadButton AndroidTranslateGamepadButton(int button)
{
    switch (button)
    {
...
static int32_t AndroidInputCallback(struct android_app *app, AInputEvent *event)
{
    ...
    else if (type == AINPUT_EVENT_TYPE_KEY)
    {
            ...
            !FLAG_IS_SET(source, AINPUT_SOURCE_KEYBOARD))
        {
            ...
            rl_GamepadButton button = AndroidTranslateGamepadButton(keycode);
        ...
        rl_KeyboardKey key = ((keycode > 0) && (keycode < KEYCODE_MAP_SIZE))? mapKeycode[keycode] : KEY_NULL;
        if (key != KEY_NULL)
        {
    ...
    for (int i = 0; (i < touchRaw.pointCount) && (i < MAX_TOUCH_POINTS); i++)
    {
        ...
        touchRaw.position[i] = (rl_Vector2){ AMotionEvent_getX(event, i), AMotionEvent_getY(event, i) };
    ...
    for (int i = 0; (i < touchRaw.pointCount) && (i < MAX_TOUCH_POINTS); i++)
    {
        ...
        gestureEvent.position[gestureEvent.pointCount].x /= (float)rl_GetScreenWidth();
        gestureEvent.position[gestureEvent.pointCount].y /= (float)rl_GetScreenHeight();
