...
static rl_KeyboardKey ConvertScancodeToKey(u32 keycode);
...
void rl_SetWindowIcons(rl_Image *images, int count)
{
    ...
    {
        rl_Image *bigIcon = NULL;
        rl_Image *smallIcon = NULL;
...
rl_Vector2 rl_GetMonitorPosition(int monitor)
{
    ...
    return (rl_Vector2){ (float)mons[monitor].x, (float)mons[monitor].y };
}
...
{
    if (platform.window == NULL) return (rl_Vector2){ 0.0f, 0.0f };
    return (rl_Vector2){ (float)platform.window->r.x, (float)platform.window->r.y };
}
...
rl_Vector2 rl_GetWindowScaleDPI(void)
{
    ...
    return (rl_Vector2){ monitor.scaleX, monitor.scaleX };
}
...
rl_Image rl_GetClipboardImage(void)
{
    ...
    else image = rl_LoadImageFromMemory(".bmp", (const unsigned char *)fileData, dataSize);
#else
    TRACELOG(LOG_WARNING, "Clipboard image: PLATFORM_DESKTOP_RGFW doesn't implement rl_GetClipboardImage() for this OS");
#endif
...
{
    TRACELOG(LOG_WARNING, "rl_SetGamepadMappings() unsupported on target platform");
    return 0;
}
...
{
    TRACELOG(LOG_WARNING, "rl_SetGamepadVibration() unsupported on target platform");
}
...
{
    TRACELOG(LOG_WARNING, "rl_GetKeyName() unsupported on target platform");
...
void rl_PollInputEvents(void)
{
    ...
    if (FLAG_IS_SET(platform.window->_flags, RGFW_HOLD_MOUSE))
    {
        ...
        CORE.Input.Mouse.currentPosition = (rl_Vector2){ 0.0f, 0.0f };
    }
    else
    {
        CORE.Input.Mouse.previousPosition = CORE.Input.Mouse.currentPosition;
    ...
    if ((CORE.Window.eventWaiting) || (rl_IsWindowState(FLAG_WINDOW_MINIMIZED) && !rl_IsWindowState(FLAG_WINDOW_ALWAYS_RUN)))
    {
    ...
    while (RGFW_window_checkEvent(platform.window))
    {
        ...
        switch (event->type)
        {
            ...
            case RGFW_windowResized:
            {
                ...
                if (rl_IsWindowState(FLAG_WINDOW_HIGHDPI))
                {
                    CORE.Window.screen.width = (int)(platform.window->r.w/rl_GetWindowScaleDPI().x);
                    CORE.Window.screen.height = (int)(platform.window->r.h/rl_GetWindowScaleDPI().y);
                }
                else
                {
            ...
            {
                rl_KeyboardKey key = ConvertScancodeToKey(event->key);
                if (key != KEY_NULL)
                {
...
int InitPlatform(void)
{
    ...
    if (platform.window != NULL)
    {
        ...
        // be set with rl_SetExitKey()) and defaults to Escape
...
static rl_KeyboardKey ConvertScancodeToKey(u32 keycode)
{
    ...
    return (rl_KeyboardKey)keyMappingRGFW[keycode];
}
