...
// Support retrieving native window handlers
    ...
    #include <sys/time.h>               // Required for: timespec, nanosleep(), select() - POSIX
            ...
            #define GLFW_EXPOSE_NATIVE_X11
                #define rl_Font X11Font    // Hack to fix 'rl_Font' name collision
                                        // The definition and references to the X11 rl_Font type will be replaced by 'X11Font'
                                        // Works as long as the current file consistently references any X11 rl_Font as X11Font
        ...
        #include "GLFW/glfw3native.h"   // Include native header only once, regardless of how many backends are defined
            ...
            #undef rl_Font                 // Revert hack and allow normal raylib rl_Font usage
        #endif
    #endif
#endif
...
void rl_ToggleFullscreen(void)
{
    if (!FLAG_IS_SET(CORE.Window.flags, FLAG_FULLSCREEN_MODE))
    {
        ...
        int monitorCount = 0;
        int monitorIndex = rl_GetCurrentMonitor();
    ...
    else
    {
        ...
        // Set fullscreen flag to be processed on FramebufferSizeCallback() accordingly
        // and considered by rl_GetWindowScaleDPI()
        ...
        {
            rl_Vector2 scaleDpi = rl_GetWindowScaleDPI();
...
void rl_ToggleBorderlessWindowed(void)
{
    ...
    if (FLAG_IS_SET(CORE.Window.flags, FLAG_FULLSCREEN_MODE)) rl_ToggleFullscreen();

    int monitorCount = 0;
    GLFWmonitor **monitors = glfwGetMonitors(&monitorCount);
    const int monitor = rl_GetCurrentMonitor();
...
void rl_SetWindowState(unsigned int flags)
{
    ...
    // NOTE: This must be handled before FLAG_FULLSCREEN_MODE because rl_ToggleBorderlessWindowed() needs to get some fullscreen values if fullscreen is running
    ...
    {
        rl_ToggleBorderlessWindowed();     // NOTE: Window state flag updated inside function
    }
    ...
    {
        rl_ToggleFullscreen();     // NOTE: Window state flag updated inside function
    }
    ...
    {
        //GLFW_ICONIFIED
        rl_MinimizeWindow();       // NOTE: Window state flag updated inside function
    }
    ...
    {
        //GLFW_MAXIMIZED
        rl_MaximizeWindow();       // NOTE: Window state flag updated inside function
    }
...
void rl_ClearWindowState(unsigned int flags)
{
    ...
    {
        rl_RestoreWindow();       // NOTE: Window state flag updated inside function
    }
...
// Set icon for window
// NOTE 1: rl_Image must be in RGBA format, 8bit per channel
// NOTE 2: rl_Image is scaled by the OS for all required sizes
...
// Storing the handle locally and returning a pointer in rl_GetWindowHandle allows the code to work regardless of pointer width
...
{
    return (rl_Vector2){ (float)CORE.Window.position.x, (float)CORE.Window.position.y };
}
...
rl_Image rl_GetClipboardImage(void)
{
    ...
    else image = rl_LoadImageFromMemory(".bmp", (const unsigned char *)fileData, (int)dataSize);
#else
...
{
    TRACELOG(LOG_WARNING, "rl_SetGamepadVibration() not available on target platform");
}
...
void rl_SetMouseCursor(int cursor)
{
    ...
    {
        // NOTE: We are relating internal GLFW enum values to our rl_MouseCursor enum values
...
void rl_PollInputEvents(void)
{
    ...
    for (int i = 0; i < MAX_GAMEPADS; i++)
    {
        if (CORE.Input.Gamepad.ready[i])     // Check if gamepad is available
        {
            ...
            {
                int button = -1;        // rl_GamepadButton enum values assigned
...
int InitPlatform(void)
{
    ...
    // Forcing this initialization here avoids doing it on rl_PollInputEvents() called by rl_EndDrawing() after first frame has been just drawn
    ...
    if (CORE.Window.ready)
    {
        ...
        if (FLAG_IS_SET(CORE.Window.flags, FLAG_WINDOW_HIGHDPI))
        {
            ...
            rl_SetMouseScale((float)CORE.Window.screen.width/fbWidth, (float)CORE.Window.screen.height/fbHeight);
#endif
        }
        ...
        rl_SetWindowPosition(CORE.Window.position.x, CORE.Window.position.y);
    ...
    rl_SetWindowState(requestedWindowFlags);
...
// NOTE: Window resizing not enabled by default, use rl_SetConfigFlags()
...
#if !defined(__APPLE__)
            // Mouse input scaling for the new screen size
            rl_SetMouseScale(1.0f/scaleDpi.x, 1.0f/scaleDpi.y);
#endif
        }
        else
        {
...
#if !defined(__APPLE__)
    // Mouse input scaling for the new screen size
    rl_SetMouseScale(1.0f/scalex, 1.0f/scaley);
#endif
