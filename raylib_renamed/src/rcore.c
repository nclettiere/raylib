...
typedef struct CoreData {
    ...
    struct {
        ...
        struct {
            ...
            rl_Vector2 lockedPosition;         // Mouse position when locked
...
//----------------------------------------------------------------------------------
rl_RLAPI const char *raylib_version = rl_RAYLIB_VERSION;  // raylib version exported symbol, required for some bindings
...
__declspec(dllimport) void __stdcall Sleep(unsigned long msTimeout); // Required for: rl_WaitTime()
#endif
...
{
    TRACELOG(LOG_INFO, "Initializing raylib %s", rl_RAYLIB_VERSION);
...
void rl_BeginMode3D(rl_Camera camera)
{
    ...
    if (camera.projection == CAMERA_PERSPECTIVE)
    {
        ...
        double top = rlGetCullDistanceNear()*tan(camera.fovy*0.5*rl_DEG2RAD);
...
void rl_BeginTextureMode(rl_RenderTexture2D target)
{
    ...
    // calculation when using rl_BeginTextureMode()
...
rl_Shader rl_LoadShader(const char *vsFileName, const char *fsFileName)
{
    ...
    if ((vShaderStr == NULL) && (fShaderStr == NULL)) TRACELOG(LOG_WARNING, "SHADER: rl_Shader files provided are not valid, using default shader");
...
rl_Shader rl_LoadShaderFromMemory(const char *vsCode, const char *fsCode)
{
    ...
    if (shader.id == 0)
    {
        // rl_Shader could not be loaded but we still load the location points to avoid potential crashes
...
bool rl_IsShaderValid(rl_Shader shader)
{
...
rl_Ray rl_GetScreenToWorldRayEx(rl_Vector2 position, rl_Camera camera, int width, int height)
{
    ...
    if (camera.projection == CAMERA_PERSPECTIVE)
    {
        ...
        matProj = MatrixPerspective(camera.fovy*rl_DEG2RAD, ((double)width/(double)height), rlGetCullDistanceNear(), rlGetCullDistanceFar());
    }
    else if (camera.projection == CAMERA_ORTHOGRAPHIC)
    {
...
rl_Matrix rl_GetCameraMatrix2D(rl_Camera2D camera)
{
    ...
    rl_Matrix matRotation = MatrixRotate((rl_Vector3){ 0.0f, 0.0f, 1.0f }, camera.rotation*rl_DEG2RAD);
...
int *rl_LoadRandomSequence(unsigned int count, int min, int max)
{
    ...
    {
        value = rl_GetRandomValue(min, max);
        dupValue = false;
...
void rl_TakeScreenshot(const char *fileName)
{
    ...
    if (FLAG_IS_SET(CORE.Window.flags, FLAG_WINDOW_HIGHDPI)) scale = rl_GetWindowScaleDPI();
    ...
    strncpy(path, rl_TextFormat("%s/%s", CORE.Storage.basePath, fileName), MAX_FILEPATH_LENGTH - 1);
...
{
    if (CORE.Window.ready) TRACELOG(LOG_WARNING, "WINDOW: rl_SetConfigFlags called after window initialization, Use \"rl_SetWindowState\" to set flags instead");
...
int rl_FileCopy(const char *srcPath, const char *dstPath)
{
    int result = 0;
    int srcDataSize = 0;
    unsigned char *srcFileData = rl_LoadFileData(srcPath, &srcDataSize);

    // Create required paths if they do not exist
    if (!rl_DirectoryExists(rl_GetDirectoryPath(dstPath)))
        result = rl_MakeDirectory(rl_GetDirectoryPath(dstPath));

    if (result == 0) // Directory created successfully (or already exists)
    {
        if ((srcFileData != NULL) && (srcDataSize > 0))
            result = rl_SaveFileData(dstPath, srcFileData, srcDataSize);
    }

    rl_UnloadFileData(srcFileData);
...
int rl_FileMove(const char *srcPath, const char *dstPath)
{
    ...
    if (rl_FileExists(srcPath))
    {
        rl_FileCopy(srcPath, dstPath);
        rl_FileRemove(srcPath);
    }
    else result = -1;
...
int rl_FileTextReplace(const char *fileName, const char *search, const char *replacement)
{
    ...
    if (rl_FileExists(fileName))
    {
        fileText = rl_LoadFileText(fileName);
        fileTextUpdated = rl_TextReplace(fileText, search, replacement);
        result = rl_SaveFileText(fileName, fileTextUpdated);
        rl_MemFree(fileTextUpdated);
...
const char *rl_GetFileNameWithoutExt(const char *filePath)
{
    ...
    if (filePath != NULL)
    {
        strncpy(fileName, rl_GetFileName(filePath), MAX_FILENAME_LENGTH - 1); // Get filename.ext without path
...
int rl_MakeDirectory(const char *dirPath)
{
    ...
    if (!rl_DirectoryExists(dirPath)) return -1;
...
bool rl_ExportAutomationEventList(rl_AutomationEventList list, const char *fileName)
{
    ...
    */
    ...
    // NOTE: Save to memory buffer and rl_SaveFileText()
