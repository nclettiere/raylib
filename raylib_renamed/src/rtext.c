...
*           Load default raylib font on initialization to be used by rl_DrawText() and rl_MeasureText()
*           If no default font loaded, rl_DrawTextEx() and rl_MeasureTextEx() are required
*
...
*           rl_TextSplit() function static buffer max size
*
*       #define MAX_TEXTSPLIT_COUNT
*           rl_TextSplit() function static substrings pointers array (pointing to static buffer)
*
*   DEPENDENCIES:
...
#include "rlgl.h"           // OpenGL abstraction layer to OpenGL 1.1, 2.1, 3.3+ or ES2 -> Only rl_DrawTextPro()

#include <stdlib.h>         // Required for: malloc(), free()
#include <stdio.h>          // Required for: vsprintf()
#include <string.h>         // Required for: strcmp(), strstr(), strncpy() [Used in rl_TextReplace()], sscanf() [Used in LoadBMFont()]
#include <stdarg.h>         // Required for: va_list, va_start(), vsprintf(), va_end() [Used in rl_TextFormat()]
#include <ctype.h>          // Required for: toupper(), tolower() [Used in rl_TextToUpper(), rl_TextToLower()]
...
// NOTE: Default font is loaded on rl_InitWindow() and disposed on rl_CloseWindow() [module: core]
static rl_Font defaultFont = { 0 };
#endif
...
static rl_Font LoadBMFont(const char *fileName);   // Load a BMFont file (AngelCode font file)
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
static rl_GlyphInfo *LoadFontDataBDF(const unsigned char *fileData, int dataSize, const int *codepoints, int codepointCount, int *outFontSize);
#endif
...
extern void LoadFontDefault(void)
{
    ...
    // This way, we reconstruct rl_Font without creating large global variables
    ...
    defaultFont.texture = rl_LoadTextureFromImage(imFont);
    ...
    // NOTE: This memory must be freed at end! --> Done by rl_CloseWindow()
    defaultFont.glyphs = (rl_GlyphInfo *)RL_CALLOC(defaultFont.glyphCount, sizeof(rl_GlyphInfo));
    defaultFont.recs = (rl_Rectangle *)RL_CALLOC(defaultFont.glyphCount, sizeof(rl_Rectangle));
    ...
    for (int i = 0; i < defaultFont.glyphCount; i++)
    {
        ...
        defaultFont.glyphs[i].image = rl_ImageFromImage(imFont, defaultFont.recs[i]);
    }
...
extern void UnloadFontDefault(void)
{
    for (int i = 0; i < defaultFont.glyphCount; i++) rl_UnloadImage(defaultFont.glyphs[i].image);
    rl_UnloadTexture(defaultFont.texture);
...
// Load rl_Font from file into GPU memory (VRAM)
rl_Font rl_LoadFont(const char *fileName)
{
...
#if defined(SUPPORT_FILEFORMAT_TTF)
    if (rl_IsFileExtension(fileName, ".ttf") || rl_IsFileExtension(fileName, ".otf")) font = rl_LoadFontEx(fileName, FONT_TTF_DEFAULT_SIZE, NULL, FONT_TTF_DEFAULT_NUMCHARS);
    else
#endif
#if defined(SUPPORT_FILEFORMAT_FNT)
    if (rl_IsFileExtension(fileName, ".fnt")) font = LoadBMFont(fileName);
    else
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
    if (rl_IsFileExtension(fileName, ".bdf")) font = rl_LoadFontEx(fileName, FONT_TTF_DEFAULT_SIZE, NULL, FONT_TTF_DEFAULT_NUMCHARS);
    else
#endif
    {
        rl_Image image = rl_LoadImage(fileName);
        if (image.data != NULL) font = rl_LoadFontFromImage(image, rl_MAGENTA, FONT_TTF_DEFAULT_FIRST_CHAR);
        else font = rl_GetFontDefault();
    ...
    {
        rl_SetTextureFilter(font.texture, TEXTURE_FILTER_POINT); // By default, we set point filter (the best performance)
...
// Load rl_Font from TTF or BDF font file with generation parameters
// NOTE: You can pass an array with desired characters, those characters should be available in the font
// if array is NULL, default char set is selected 32..126
rl_Font rl_LoadFontEx(const char *fileName, int fontSize, const int *codepoints, int codepointCount)
{
    rl_Font font = { 0 };
    ...
    if (fileData != NULL)
    {
        ...
        font = rl_LoadFontFromMemory(rl_GetFileExtension(fileName), fileData, dataSize, fontSize, codepoints, codepointCount);
...
// Load an rl_Image font file (XNA style)
rl_Font rl_LoadFontFromImage(rl_Image image, rl_Color key, int firstChar)
{
    ...
    font.texture = rl_LoadTextureFromImage(fontClear); // Convert processed image to OpenGL texture
    font.glyphCount = index;
    font.glyphPadding = 0;

    // We got tempCharValues and tempCharsRecs populated with glyphs data
    // Now we move temp data to sized charValues and charRecs arrays
    font.glyphs = (rl_GlyphInfo *)RL_MALLOC(font.glyphCount*sizeof(rl_GlyphInfo));
    ...
    for (int i = 0; i < font.glyphCount; i++)
    {
        ...
        // Fill character image data from fontClear data
        font.glyphs[i].image = rl_ImageFromImage(fontClear, tempCharRecs[i]);
    }

    rl_UnloadImage(fontClear);     // Unload processed image once converted to texture
...
rl_Font rl_LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, const int *codepoints, int codepointCount)
{
    rl_Font font = { 0 };

    char fileExtLower[16] = { 0 };
    strncpy(fileExtLower, rl_TextToLower(fileType), 16 - 1);

    font.baseSize = fontSize;
    font.glyphPadding = 0;

#if defined(SUPPORT_FILEFORMAT_TTF)
    if (rl_TextIsEqual(fileExtLower, ".ttf") ||
        rl_TextIsEqual(fileExtLower, ".otf"))
    {
        font.glyphs = rl_LoadFontData(fileData, dataSize, font.baseSize, codepoints, (codepointCount > 0)? codepointCount : 95, FONT_DEFAULT, &font.glyphCount);
    }
    else
#endif
#if defined(SUPPORT_FILEFORMAT_BDF)
    if (rl_TextIsEqual(fileExtLower, ".bdf"))
    {
    ...
    if (font.glyphs != NULL)
    {
        ...
        // Update glyphs[i].image to use alpha, required to be used on rl_ImageDrawText()
        for (int i = 0; i < font.glyphCount; i++)
        {
...
bool rl_IsFontValid(rl_Font font)
{
...
rl_GlyphInfo *rl_LoadFontData(const unsigned char *fileData, int dataSize, int fontSize, const int *codepoints, int codepointCount, int type, int *glyphCount)
{
    ...
    if (fileData != NULL)
    {
        ...
        if (stbtt_InitFont(&fontInfo, (unsigned char *)fileData, 0))     // Initialize font for data reading
        {
            ...
            glyphs = (rl_GlyphInfo *)RL_CALLOC(glyphCounter, sizeof(rl_GlyphInfo));
...
#if defined(SUPPORT_FILEFORMAT_TTF) || defined(SUPPORT_FILEFORMAT_BDF)
rl_Image rl_GenImageFontAtlas(const rl_GlyphInfo *glyphs, rl_Rectangle **glyphRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    rl_Image atlas = { 0 };
...
void rl_UnloadFontData(rl_GlyphInfo *glyphs, int glyphCount)
{
    if (glyphs != NULL)
    {
        for (int i = 0; i < glyphCount; i++) rl_UnloadImage(glyphs[i].image);

        RL_FREE(glyphs);
    }
}

// Unload rl_Font from GPU memory (VRAM)
void rl_UnloadFont(rl_Font font)
{
    // NOTE: Make sure font is not default font (fallback)
    if (font.texture.id != rl_GetFontDefault().texture.id)
    {
        rl_UnloadFontData(font.glyphs, font.glyphCount);
        rl_UnloadTexture(font.texture);
        RL_FREE(font.recs);
...
bool rl_ExportFontAsCode(rl_Font font, const char *fileName)
{
    bool success = false;
    ...
    strncpy(fileNamePascal, rl_TextToPascal(rl_GetFileNameWithoutExt(fileName)), 256 - 1);
    ...
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "// FontAsCode exporter v1.0 - rl_Font data exported as an array of bytes                 //\n");
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    ...
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "// rl_Font name:    ....                                                                 //\n");
    byteCount += sprintf(txtData + byteCount, "// rl_Font creator: ....                                                                 //\n");
    byteCount += sprintf(txtData + byteCount, "// rl_Font LICENSE: ....                                                                 //\n");
    byteCount += sprintf(txtData + byteCount, "//                                                                                    //\n");
    byteCount += sprintf(txtData + byteCount, "////////////////////////////////////////////////////////////////////////////////////////\n\n");

    // Support font export and initialization
    // NOTE: This mechanism is highly coupled to raylib
    rl_Image image = rl_LoadImageFromTexture(font.texture);
    if (image.format != PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) TRACELOG(LOG_WARNING, "rl_Font export as code: rl_Font image format is not rl_GRAY+ALPHA!");
    int imageDataSize = rl_GetPixelDataSize(image.width, image.height, image.format);

    // rl_Image data is usually GRAYSCALE + ALPHA and can be reduced to GRAYSCALE
    //rl_ImageFormat(&image, PIXELFORMAT_UNCOMPRESSED_GRAYSCALE);

#define SUPPORT_COMPRESSED_FONT_ATLAS
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
    // WARNING: Data is compressed using raylib rl_CompressData() DEFLATE,
    // it requires to be decompressed with raylib rl_DecompressData(), that requires
    // compiling raylib with SUPPORT_COMPRESSION_API config flag enabled

    // Compress font image data
    int compDataSize = 0;
    unsigned char *compData = rl_CompressData((const unsigned char *)image.data, imageDataSize, &compDataSize);

    // Save font image data (compressed)
    byteCount += sprintf(txtData + byteCount, "#define COMPRESSED_DATA_SIZE_FONT_%s %i\n\n", rl_TextToUpper(fileNamePascal), compDataSize);
    byteCount += sprintf(txtData + byteCount, "// rl_Font image pixels data compressed (DEFLATE)\n");
    byteCount += sprintf(txtData + byteCount, "// NOTE: Original pixel data simplified to GRAYSCALE\n");
    byteCount += sprintf(txtData + byteCount, "static unsigned char fontData_%s[COMPRESSED_DATA_SIZE_FONT_%s] = { ", fileNamePascal, rl_TextToUpper(fileNamePascal));
    ...
    byteCount += sprintf(txtData + byteCount, "// rl_Font image pixels data\n");
    ...
    // Save font recs data
    byteCount += sprintf(txtData + byteCount, "// rl_Font characters rectangles data\n");
    byteCount += sprintf(txtData + byteCount, "static rl_Rectangle fontRecs_%s[%i] = {\n", fileNamePascal, font.glyphCount);
    for (int i = 0; i < font.glyphCount; i++)
    {
    ...
    byteCount += sprintf(txtData + byteCount, "// rl_Font glyphs info data\n");
    byteCount += sprintf(txtData + byteCount, "// NOTE: No glyphs.image data provided\n");
    byteCount += sprintf(txtData + byteCount, "static rl_GlyphInfo fontGlyphs_%s[%i] = {\n", fileNamePascal, font.glyphCount);
    for (int i = 0; i < font.glyphCount; i++)
    {
    ...
    byteCount += sprintf(txtData + byteCount, "// rl_Font loading function: %s\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "static rl_Font LoadFont_%s(void)\n{\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    rl_Font font = { 0 };\n\n");
...
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
    byteCount += sprintf(txtData + byteCount, "    // NOTE: Compressed font image data (DEFLATE), it requires rl_DecompressData() function\n");
    byteCount += sprintf(txtData + byteCount, "    int fontDataSize_%s = 0;\n", fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    unsigned char *data = rl_DecompressData(fontData_%s, COMPRESSED_DATA_SIZE_FONT_%s, &fontDataSize_%s);\n", fileNamePascal, rl_TextToUpper(fileNamePascal), fileNamePascal);
    byteCount += sprintf(txtData + byteCount, "    rl_Image imFont = { data, %i, %i, 1, %i };\n\n", image.width, image.height, image.format);
#else
    byteCount += sprintf(txtData + byteCount, "    rl_Image imFont = { fontImageData_%s, %i, %i, 1, %i };\n\n", styleName, image.width, image.height, image.format);
#endif
    byteCount += sprintf(txtData + byteCount, "    // Load texture from image\n");
    byteCount += sprintf(txtData + byteCount, "    font.texture = rl_LoadTextureFromImage(imFont);\n");
#if defined(SUPPORT_COMPRESSED_FONT_ATLAS)
    byteCount += sprintf(txtData + byteCount, "    rl_UnloadImage(imFont);  // Uncompressed data can be unloaded from memory\n\n");
#endif
    ...
    //  - 1. Data copy. This option consumes more memory and rl_Font MUST be unloaded by user, requiring additional code
    //  - 2. Data assignment. This option consumes less memory and rl_Font MUST NOT be unloaded by user because data is on protected DATA segment
//#define SUPPORT_FONT_DATA_COPY
    ...
    byteCount += sprintf(txtData + byteCount, "    // NOTE: Required to avoid issues if trying to free font\n");
    byteCount += sprintf(txtData + byteCount, "    font.glyphs = (rl_GlyphInfo *)malloc(font.glyphCount*sizeof(rl_GlyphInfo));\n");
    byteCount += sprintf(txtData + byteCount, "    memcpy(font.glyphs, fontGlyphs_%s, font.glyphCount*sizeof(rl_GlyphInfo));\n\n", fileNamePascal);
#else
    ...
    if (success != 0) TRACELOG(LOG_INFO, "FILEIO: [%s] rl_Font as code exported successfully", fileName);
...
{
    rl_Color color = rl_LIME;                         // Good FPS
    int fps = rl_GetFPS();

    if ((fps < 30) && (fps >= 15)) color = rl_ORANGE;  // Warning FPS
    else if (fps < 15) color = rl_RED;             // Low FPS

    rl_DrawText(rl_TextFormat("%2i FPS", fps), posX, posY, 20, color);
}

// Draw text (using default font)
// NOTE: fontSize work like in any drawing program but if fontSize is lower than font-base-size, then font-base-size is used
// NOTE: chars spacing is proportional to fontSize
void rl_DrawText(const char *text, int posX, int posY, int fontSize, rl_Color color)
{
    // Check if default font has been loaded
    if (rl_GetFontDefault().texture.id != 0)
    {
        ...
        rl_DrawTextEx(rl_GetFontDefault(), text, position, (float)fontSize, (float)spacing, color);
    }
}

// Draw text using rl_Font
// NOTE: chars spacing is NOT proportional to fontSize
void rl_DrawTextEx(rl_Font font, const char *text, rl_Vector2 position, float fontSize, float spacing, rl_Color tint)
{
    if (font.texture.id == 0) font = rl_GetFontDefault();  // Security check in case of not valid font

    int size = rl_TextLength(text);    // Total size in bytes of the text, scanned by codepoints in loop
    ...
    for (int i = 0; i < size;)
    {
        ...
        int codepoint = rl_GetCodepointNext(&text[i], &codepointByteCount);
        int index = rl_GetGlyphIndex(font, codepoint);

        if (codepoint == '\n')
        {
            // NOTE: Line spacing is a global variable, use rl_SetTextLineSpacing() to setup
        ...
        else
        {
            ...
            {
                rl_DrawTextCodepoint(font, codepoint, (rl_Vector2){ position.x + textOffsetX, position.y + textOffsetY }, fontSize, tint);
            }
...
// Draw text using rl_Font and pro parameters (rotation)
void rl_DrawTextPro(rl_Font font, const char *text, rl_Vector2 position, rl_Vector2 origin, float rotation, float fontSize, float spacing, rl_Color tint)
{
    rlPushMatrix();
        ...
        rl_DrawTextEx(font, text, (rl_Vector2){ 0.0f, 0.0f }, fontSize, spacing, tint);

    rlPopMatrix();
}

// Draw one character (codepoint)
void rl_DrawTextCodepoint(rl_Font font, int codepoint, rl_Vector2 position, float fontSize, rl_Color tint)
{
    ...
    rl_DrawTexturePro(font.texture, srcRec, dstRec, (rl_Vector2){ 0, 0 }, 0.0f, tint);
}

// Draw multiple character (codepoints)
void rl_DrawTextCodepoints(rl_Font font, const int *codepoints, int codepointCount, rl_Vector2 position, float fontSize, float spacing, rl_Color tint)
{
    float textOffsetY = 0;          // Offset between lines (on linebreak '\n')
    float textOffsetX = 0.0f;       // Offset X to next character to draw
    ...
    for (int i = 0; i < codepointCount; i++)
    {
        int index = rl_GetGlyphIndex(font, codepoints[i]);
        ...
        else
        {
            ...
            {
                rl_DrawTextCodepoint(font, codepoints[i], (rl_Vector2){ position.x + textOffsetX, position.y + textOffsetY }, fontSize, tint);
            }
...
int rl_MeasureText(const char *text, int fontSize)
{
    ...
    if (rl_GetFontDefault().texture.id != 0)
    {
        int defaultFontSize = 10;   // Default rl_Font glyphs height in pixel
        if (fontSize < defaultFontSize) fontSize = defaultFontSize;
        int spacing = fontSize/defaultFontSize;

        textSize = rl_MeasureTextEx(rl_GetFontDefault(), text, (float)fontSize, (float)spacing);
    }

    return (int)textSize.x;
}

// Measure string size for rl_Font
rl_Vector2 rl_MeasureTextEx(rl_Font font, const char *text, float fontSize, float spacing)
{
    rl_Vector2 textSize = { 0 };

    if ((font.texture.id == 0) || (text == NULL) || (text[0] == '\0')) return textSize; // Security check

    int size = rl_TextLength(text);    // Get size in bytes of text
    ...
    for (int i = 0; i < size;)
    {
        ...
        int codepointByteCount = 0;
        letter = rl_GetCodepointNext(&text[i], &codepointByteCount);
...
int rl_GetGlyphIndex(rl_Font font, int codepoint)
{
    int index = 0;
    if (!rl_IsFontValid(font)) return index;
...
// NOTE: If codepoint is not found in the font it fallbacks to '?'
rl_GlyphInfo rl_GetGlyphInfo(rl_Font font, int codepoint)
{
    rl_GlyphInfo info = { 0 };

    info = font.glyphs[rl_GetGlyphIndex(font, codepoint)];
...
// NOTE: If codepoint is not found in the font it fallbacks to '?'
rl_Rectangle rl_GetGlyphAtlasRec(rl_Font font, int codepoint)
{
    rl_Rectangle rec = { 0 };

    rec = font.recs[rl_GetGlyphIndex(font, codepoint)];
...
char **rl_LoadTextLines(const char *text, int *count)
{
    ...
    if (text != NULL)
    {
        int textLength = rl_TextLength(text);
        lineCount = 1;
...
// Unload text lines
void rl_UnloadTextLines(char **lines, int lineCount)
{
...
char *rl_GetTextBetween(const char *text, const char *begin, const char *end)
{
    ...
    int beginIndex = rl_TextFindIndex(text, begin);

    if (beginIndex > -1)
    {
        int beginLen = rl_TextLength(begin);
        int endIndex = rl_TextFindIndex(text + beginIndex + beginLen, end);
...
char *rl_TextReplace(const char *text, const char *search, const char *replacement)
{
    ...
    if ((text != NULL) && (search != NULL))
    {
        ...
        textLen = rl_TextLength(text);
        searchLen = rl_TextLength(search);
        if (searchLen == 0) return NULL;  // Empty search causes infinite loop during count

        replaceLen = rl_TextLength(replacement);
    ...
    if ((text != NULL) && (begin != NULL) && (end != NULL))
    {
        ...
        if (beginIndex > -1)
        {
            ...
            if (endIndex > -1)
            {
                ...
                int textLen = rl_TextLength(text);
                int replaceLen = (replacement == NULL)? 0 : rl_TextLength(replacement);
...
char *rl_TextInsert(const char *text, const char *insert, int position)
{
    ...
    {
        int textLen = rl_TextLength(text);
        int insertLen = rl_TextLength(insert);
...
char *rl_TextJoin(char **textList, int count, const char *delimiter)
{
    ...
    int totalLength = 0;
    int delimiterLen = rl_TextLength(delimiter);

    for (int i = 0; i < count; i++)
    {
        int textLength = rl_TextLength(textList[i]);
...
void rl_TextAppend(char *text, const char *append, int *position)
{
    ...
    {
        rl_TextCopy(text + *position, append);
        *position += rl_TextLength(append);
    }
}
...
char *rl_LoadUTF8(const int *codepoints, int length)
{
    ...
    if ((codepoints != NULL) && (length > 0))
    {
        ...
        {
            utf8 = rl_CodepointToUTF8(codepoints[i], &bytes);
...
int *rl_LoadCodepoints(const char *text, int *count)
{
    ...
    if (text != NULL)
    {
        ...
        {
            codepoints[codepointCount] = rl_GetCodepointNext(text + i, &codepointSize);
            i += codepointSize;
        }
...
int rl_GetCodepointPrevious(const char *text, int *codepointSize)
{
    ...
    *codepointSize = 1;
    ...
    int cpSize = 0;
    codepoint = rl_GetCodepointNext(ptr, &cpSize);
...
static rl_Font LoadBMFont(const char *fileName)
{
    ...
    if (readVars < 4) { rl_UnloadFileText(fileText); return font; } // Some data not available, file malformed

    if (pageCount > MAX_FONT_IMAGE_PAGES)
    {
        TRACELOG(LOG_WARNING, "FONT: [%s] rl_Font defines more pages than supported: %i/%i", fileName, pageCount, MAX_FONT_IMAGE_PAGES);
    ...
    for (int i = 0; i < pageCount; i++)
    {
        ...
        if (readVars < 1) { rl_UnloadFileText(fileText); return font; } // No fileName read
    }
    ...
    if (readVars < 1) { rl_UnloadFileText(fileText); return font; } // No glyphCount read

    // Load all required images for further compose
    rl_Image *imFonts = (rl_Image *)RL_CALLOC(pageCount, sizeof(rl_Image)); // rl_Font atlases, multiple images

    for (int i = 0; i < pageCount; i++)
    {
        imFonts[i] = rl_LoadImage(rl_TextFormat("%s/%s", rl_GetDirectoryPath(fileName), imFileName[i]));

        if (imFonts[i].format == PIXELFORMAT_UNCOMPRESSED_GRAYSCALE)
        {
            // Convert image to GRAYSCALE + ALPHA, using the mask as the alpha channel
            rl_Image imFontAlpha = {
            ...
            rl_UnloadImage(imFonts[i]);
            imFonts[i] = imFontAlpha;
        }
    }

    rl_Image fullFont = imFonts[0];
    for (int i = 1; i < pageCount; i++) rl_UnloadImage(imFonts[i]);
    ...
    if (pageCount > 1)
    {
        ...
        rl_ImageResizeCanvas(&fullFont, imWidth, imHeight*pageCount, 0, 0, rl_BLACK);
        ...
        for (int i = 1; i < pageCount; i++)
        {
            ...
            rl_ImageDraw(&fullFont, imFonts[i], srcRec, destRec, rl_WHITE);
        }
    }

    RL_FREE(imFonts);

    font.texture = rl_LoadTextureFromImage(fullFont);

    // Fill font characters info data
    font.baseSize = fontSize;
    font.glyphCount = glyphCount;
    font.glyphPadding = 0;
    font.glyphs = (rl_GlyphInfo *)RL_MALLOC(glyphCount*sizeof(rl_GlyphInfo));
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        if (readVars == 9)  // Make sure all char data has been properly read
        {
            ...
            font.glyphs[i].image = rl_ImageFromImage(fullFont, font.recs[i]);
        }
        else
        {
            font.glyphs[i].image = rl_GenImageColor((int)font.recs[i].width, (int)font.recs[i].height, rl_BLACK);
            TRACELOG(LOG_WARNING, "FONT: [%s] Some characters data not correctly provided", fileName);
        }
    }

    rl_UnloadImage(fullFont);
    ...
    else TRACELOG(LOG_INFO, "FONT: [%s] rl_Font loaded successfully (%i glyphs)", fileName, font.glyphCount);
...
static rl_GlyphInfo *LoadFontDataBDF(const unsigned char *fileData, int dataSize, const int *codepoints, int codepointCount, int *outFontSize)
{
    ...
    int fontBBw = 0;                // rl_Font base character bounding box width
    int fontBBh = 0;                // rl_Font base character bounding box height
    int fontBBxoff0 = 0;            // rl_Font base character bounding box X0 offset
    int fontBByoff0 = 0;            // rl_Font base character bounding box Y0 offset
    int fontAscent = 0;             // rl_Font ascent
    ...
    glyphs = (rl_GlyphInfo *)RL_CALLOC(codepointCount, sizeof(rl_GlyphInfo));
